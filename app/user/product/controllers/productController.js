...
exports.getUserWithProducts = async (req, res) => {
  try {
    ...
    // Pagination & Sorting
    const { sortBy = "relevance", page = 1, limit = 20 } = req.query;
    ...
    return errorResponse(
      res,
      "Failed to fetch user and products",
      500,
      error.message
    );
  }
...
exports.getAllProducts = async (req, res) => {
  try {
    const responseProducts = await Product.find({ status: "active" }).populate(
      "user",
      "userName firstName lastName profile lastLoginAt city country"
    );

    const responseData = responseProducts.map(getAllProductRes); // Format response
    console.log("Fetched products:", responseProducts);

    return successResponse(res, "Active products fetched successfully", {
      items: responseData,
    });
  } catch (error) {
...
exports.getProduct = async (req, res) => {
  try {
    ...
    const responseProduct = await Product.findByIdAndUpdate(
      ...
      .populate(
        "user",
        "userName email profile city country lastLoginAt firstName lastName cityShow"
      );
    ...
    if (responseProduct.user && responseProduct.user._id) {
      const ratings = await Rating.find({
        ratedUser: responseProduct.user._id,
      });
  ...
  try {
    ...
    if (index > -1) {
      ...
      try {
        ...
        console.error(
          "Error sending product liked notification:",
          notificationError
        );
...
exports.filterProducts = async (req, res) => {
  try {
    ...
    const responseItems = products.map((product) => ({
      ...
      product_photos: product.product_photos.map(
        (photo) => `${process.env.BASE_URL}/${photo}`
      ),
...
exports.getAllProducts = async (req, res) => {
  try {
    ...
    if (isAuthenticated && mongoose.Types.ObjectId.isValid(req.user?.id)) {
      ...
      if (personalization) {
        const { followedCategories = [], followedBrands = [] } =
          personalization;
    ...
    const pipeline = [
      ...
      }, // include products with no user and users not deleted
      {
        $lookup: {
          from: "categories",
          localField: "category",
  ...
  try {
    ...
    if (lastBumpTime && lastBumpTime > twentyFourHoursAgo) {
      const timeLeft = Math.ceil(
        (lastBumpTime.getTime() + 24 * 60 * 60 * 1000 - now.getTime()) /
          (60 * 60 * 1000)
      );
      return errorResponse(
        res,
        `You can bump this product again in ${timeLeft} hours`,
        400
      );
    }
...
exports.markAsSold = async (req, res) => {
  try {
    ...
    await product.updateStatus(
      "sold",
      userId,
      "Product marked as sold by owner"
    );
...
exports.markAsReserved = async (req, res) => {
  try {
    ...
    await product.updateStatus(
      "reserved",
      userId,
      "Product marked as reserved by owner"
    );
    ...
    return errorResponse(
      res,
      "Failed to mark product as reserved",
      500,
      error.message
    );
  }
...
exports.reactivateProduct = async (req, res) => {
  try {
    ...
    if (!["sold", "reserved"].includes(product.status)) {
      return errorResponse(
        res,
        "Can only reactivate sold or reserved products",
        400
      );
    }
